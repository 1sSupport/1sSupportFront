...
<template>
  <div>
    <search-input @search="searchHandler($event)" :marks="articlesMarks"/>
    <search-result
      ...
      :lastQuery="lastQuery"
    />
    <modal-component :token="token" :sessionId="sessionId"/>
  </div>
</template>
  ...
  methods: {
    getToken: async function() {
      var response = await axios.post(
        "http://www.u0612907.plsk.regruhosting.ru/api/Token/CreateToken",
        {
          inn: "000000000000",
          login: "test"
        }
    ...
    startSession: async function(token) {
      let axiosConfig = {
        method: "post",
        url:
          "http://www.u0612907.plsk.regruhosting.ru/api/Session/StartSession",
        headers: {
    ...
    getArticles: async function(query, sessionId) {
      let axiosConfig = {
        method: "get",
        url:
          "http://www.u0612907.plsk.regruhosting.ru/api/Article/GetArticlesByQuery",
        headers: {
    ...
    searchHandler: async function(payload) {
      ...
      this.searchRequestResponse = await this.getArticles(
        payload,
        this.sessionId
      );
    },
