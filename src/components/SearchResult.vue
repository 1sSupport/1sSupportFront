    ...
    <v-layout row>
      <v-flex xs12>
        <v-card
          ...
          flat
          class="text-xs-left"
        >Результатов найдено: {{ searchResponse.length }}</v-card>

        <v-card flat max-width="90%">
          <v-list three-line>
            <item
              v-if="searchResponse != false"
              v-for="(item) in this.searchResponse"
              :key="item.id"
            >
              <v-list-tile-content>
                <router-link
                  class="article-title"
                  :to="{ name: 'ArticlePage', params: {articleId: item.id, token: token, sessionId: sessionId, query: lastQuery}  }"
                >
